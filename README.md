...
- [Redux Tutorial - Learn ReactRedux in one video](https://youtube.com/)
  - Topics
  ![topics_of_redux](./doc/topics_of_redux.png)

  - 2 Redux highlights
    - Redux helps apps to scale by providing a sensical way to manage state through a unidirectional data flow model.
    ![redux_highlights](./doc/redux_highlights.png)
    
    - Redux Devtools is awesome.
    ![redux_highlights](./doc/redux_highlights_devtool.png)
 

  - Store, actions, and reducers
  ![redux_flow](./doc/redux_flow.png)
  
    - Store is the entile state of the app, bascially one big object.
    
    - Actions are sent or dispatched from view which are payload read by reducers.
    
    - Reducers read the payload from actions, then update store(master state) accordingly.
  ...
  store.dispatch(action)   // send action to store
  ```
  ...
  console.log(store.getState())   // {products:Array(0),user:""}
  ```

  - Setting an initial state
  ```javascript
  // pass initial state to prepopulate store
  const store = createStore(allReducers, {
    products: [{name: 'iPhone'}],
    user: 'Michael'
  })
  
  console.log(store.getState())   // {products:Array(1),user:"Michael"}
  ```

  - Using Redux Devtools
  ```javascript
  // 3rd para is store enhancers
  ...
  - Using Provider makes the store available to the app
  ```javascript
  // give app access to redux store
  ...
  export default connect(mapStateToProps)(App)   // connect App component to the store provided by Provider component
  ```

  - mapStateToprops
  ```javascript
  // App.js
  // receive state from store, then provide as props to components
  ...
  // App.js
  import {upadateUser} from './actions/user-actions'
  ...
  // allow to dispatch action to store from component easily
  ...
  class App extends Component {
    render() {
      console.log(this.props)   // {aRandomProps:"whatever",products:Array(1),user:"Michael",userPlusProp:"Michael - whatever",onUpdateUser:f}
      return ()
    }
  }
  ...
  const mapActionsToProps = (dispatch, props) => {
    console.log(props)   // {aRandomProps:"whatever"}
  ...
  const mergeProps = (propsFromState, propsFromDispatch, ownProps) => {
    console.log(propsFromState, propsFromDispatch, ownProps)   // {products:Array(1),user:"Michael",userPlusProp:"Michale - whatever"}{onUpdateUser:f}{aRandomProps:"whatever"}
    return {}
  }
  ...
  ```javascript
  // allow to directly access dispatch method, so that we can make asyc call from actions.
  yarn add redux-thunk jquery

  // index.js
  import thunk from 'redux-thunk'
  import {applyMiddleware, compose, combineReuducers, createStore} from 'redux'

  const allStoreEnhancers = compose(
    applyMiddleware(thunk)   // must come first
    window.devToolsExtension && window.devToolsExtension()
  )
  ...
  export function apiRequest() {
    return dispatch => {
      ...
      $.ajax({
        ...
        error() {
          console.log('ERROR')
          // dispatch additional actions in response to the API request
          dispatch(showError())
        }
      })
    }
  ...
  - Using reselect/selectors for organization and performance
    ...
    // It's good to create smaller sub-selectors to maximize speed, they don't change then are not recomputed
  ...
  ![smart_vs_dumb_components](./doc/smart_vs_dumb_components.png)
    - top level smart component directly connects to store

    - sub-components receive props from smart component

    - smart component passes down all the state and action creators to sub-components

    - Minimize the number of smart component to control which one place to go to find all the logic for getting state and actions
